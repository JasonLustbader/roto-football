...
module.exports = class {
  getScoresForSeason1(seasonId) {
    ...
    Object.keys(statsByWeek).forEach(function(weekId) {
      ...
      var activeStats = statsByWeek[weekId].filter(function(stat) {
        return activePlayers.filter(function(player) { return player.playerId === stat.playerId}).length > 0;
      });
    ...
    return this.assignScoresOuter(flattenedStatsByWeek);
  }
  ...
  getCategoryScoresForSeason(seasonId, categoryId) {
    ...
    var stats = db.getStatsBySeasonIdAndCategoryId(seasonId, categoryId);
